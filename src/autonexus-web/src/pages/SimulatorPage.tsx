...
export function SimulatorPage({ currentView, onNavigate }: { currentView?: string; onNavigate?: (view: string) => void }) {
    ...
    useEffect(() => {
        const fetchStock = async () => {
            try {
                // Busca veículos À Venda (status=1)
    ...
    // Atualiza a taxa automaticamente ao trocar de banco (exceto personalizado)
    useEffect(() => {
            ...
            <div className="space-y-6 p-4 sm:p-6 max-w-5xl mx-auto">
                ...
                <div className="flex items-center justify-between border-b border-nexus-border pb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-white">Simulador de Financiamento</h1>
                            <p className="text-xs text-nexus-text-muted mt-0.5">Calcule parcelas com taxas em tempo real.</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-7 space-y-4">
                        ...
                        <div className="bg-nexus-card border border-nexus-border rounded-xl p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs text-slate-400 block mb-1">Valor do Veículo na Simulação</label>
                    ...
                    <div className="lg:col-span-5 space-y-4">
                        ...
                        {/* Card Saldo a Financiar com Botão WhatsApp */}
                        <div className="bg-blue-900/20 border border-blue-500/30 rounded-xl p-5 text-center space-y-3">
                            <p className="text-sm text-blue-200 font-medium uppercase tracking-wider">Saldo a Financiar</p>
                            ...
                            <button
                                ...
                                className="w-full mt-2 py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-2 shadow-md cursor-pointer"
                            >
                        ...
                        {/* Tabela de Parcelas com Botão de Rodar Ficha */}
                        <div className="bg-nexus-card border border-nexus-border rounded-xl p-5">
                            <h3 className="text-sm font-semibold text-white mb-4 text-center">Simulação de Parcelas</h3>

                            <div className="space-y-3">
                                {plans.map(plan => (
                                    <div key={plan.months} className="flex items-center justify-between p-3 rounded-lg bg-slate-900 border border-slate-800 hover:border-emerald-500/50 transition-colors group">
                                        <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex items-center gap-3 text-right">
                                            ...
                                            {financedAmount > 0 && (
                                                <button
                                                    ...
                                                    className="py-1.5 px-2.5 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-400 rounded-lg text-xs font-semibold transition-colors flex items-center gap-1 cursor-pointer"
                                                >
                                                    <Send size={12} />
                                                    <span className="hidden sm:inline">Rodar Ficha</span>
                                                </button>
                                            )}
                                        </div>
                                    </div>
