      ...
      <div className="space-y-6 p-4 sm:p-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-nexus-card border border-nexus-border p-4 rounded-xl">
            ...
            <p className="text-xl font-bold text-white mt-1">{formatCurrency(totalVolumeAll)}</p>
          </div>

          <div className="bg-nexus-card border border-nexus-border p-4 rounded-xl">
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Lucro Gerado para a Loja</p>
            <p className="text-xl font-bold text-emerald-400 mt-1">{formatCurrency(totalProfitAll)}</p>
          </div>

          <div className="bg-nexus-card border border-nexus-border p-4 rounded-xl">
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Total em Comissões (A Pagar)</p>
            <p className="text-xl font-bold text-amber-400 mt-1">{formatCurrency(totalCommissionsPaid)}</p>
          </div>
        </div>
              ...
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="border-b border-nexus-border bg-slate-950/60 text-slate-400 uppercase tracking-wider">
                    ...
                    <th className="py-3 px-4">Qtd. Vendas</th>
                    <th className="py-3 px-4">Volume Vendido</th>
                    <th className="py-3 px-4">Lucro Gerado</th>
                    <th className="py-3 px-4">% Comissão</th>
                    <th className="py-3 px-4">Comissão A Pagar</th>
                    <th className="py-3 px-4 text-center">Ver Carros Vendidos</th>
                  </tr>
                </thead>
                  ...
                  {commissions.map((c) => (
                    <React.Fragment key={c.sellerId}>
                      <tr className="hover:bg-slate-800/40 transition-colors">
                        ...
                        <td className="py-3 px-4 font-semibold">{c.totalSalesCount} veículo(s)</td>
                        <td className="py-3 px-4 font-medium text-slate-300">{formatCurrency(c.totalSalesVolume)}</td>
                        <td className="py-3 px-4 font-medium text-emerald-400">{formatCurrency(c.totalProfitGenerated)}</td>
                        <td className="py-3 px-4 text-slate-400">{c.commissionRatePercentage.toFixed(1)}% sob lucro</td>
                      ...
                      {/* Sub-tabela: Histórico de Vendas com Vendedor, Data e Hora Exatas */}
                                  ...
                                  <div key={idx} className="bg-slate-900 border border-slate-800 p-3 rounded-lg flex justify-between items-center text-xs">
                                    <div>
                                      ...
                                      <p className="text-[10px] text-blue-300 flex items-center gap-1 mt-1">
                                        <Calendar className="w-3 h-3" /> Vendido em: {formatDateTime(item.soldAt)}
                                      </p>
                                    </div>
